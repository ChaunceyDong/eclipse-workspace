...
public class ChessFrame extends JFrame{
    ...
    private void frameInitnew(){
		...
		//北区，欢迎词
		b1=new JLabel("    欢迎来到井字棋世界！！！",JLabel.CENTER);
    ...
    private void playchess(){
		...
		if(isWinornot()){      //判断是否已经分出胜负
			...
			b2.setText("游戏结束");
			switch(result){
				case 1: b3.setText("恭喜您，您赢了");break;
				case 2: b3.setText("真笨，您输了");break;
    	...
    	for(i=0;i<c_size;i++){
    		for(j=0;j<c_size;j++){
    			if(a[i][j]==0){
    				...
    				if(h_temp<minus)   minus=h_temp;   //找最小的分支
    				a[i][j]=0;    				
    			}
    		}
    	}
    ...
    private int newchoosenext1(){
        ...
        return plus;  ////与choosenext1的不同之处在于返回值的不同
    }
    
    
// 价值评估函数
// 定义：若格局对任何一方都不是获胜的，则h等于所有的空格放上电脑的棋子之后三子成一线的总数
// 减去所有空格放上玩家的棋子之后三子成一线的总数
//若棋局状况为电脑获胜，则价值评估值为无穷大，本程序中设为100即可
